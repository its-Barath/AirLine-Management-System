...
public class AirplanesController {
    ...
    public static void AddNewAirplane(Database database, Scanner s) throws SQLException {
        ...
        int id;
        if (planes.size() != 0) {
            id = ((Airplane)planes.get(planes.size() - 1)).getId() + 1;
        } else {
            id = 0;
        }

        airplane.setId(id);
        String insert = "INSERT INTO `airplanes`(`id`, `EconomyCapacity`, `BusinessCapacity`, `model`) VALUES ('" + airplane.getId() + "','" + airplane.getEconomyCapacity() + "','" + airplane.getBusinessCapacity() + "','" + airplane.getModel() + "');";
    ...
    public static void PrintAllPlanes(Database database) throws SQLException {
        System.out.println("---------------------------");
        Iterator var2 = getAllPlanes(database).iterator();

        while(var2.hasNext()) {
            Airplane plane = (Airplane)var2.next();
            plane.print();
        }
    ...
    public static void EditAirplane(Database database, Scanner s) throws SQLException {
        ...
        p.setModel(model);
        String update = "UPDATE `airplanes` SET `id`='" + p.getId() + "',`EconomyCapacity`='" + p.getEconomyCapacity() + "',`BusinessCapacity`='" + p.getBusinessCapacity() + "',`model`='" + p.getModel() + "' WHERE `id` = " + p.getId() + " ;";
    ...
    public static Airplane getPlaneByID(Database database, int id) throws SQLException {
        Airplane a = new Airplane();
        String get = "SELECT `id`, `EconomyCapacity`, `BusinessCapacity`, `model` FROM `airplanes` WHERE `id` = " + id + " ;";
        ResultSet rs = database.getStatement().executeQuery(get);
        rs.next();
        a.setId(rs.getInt("id"));
        a.setEconomyCapacity(rs.getInt("EconomyCapacity"));
