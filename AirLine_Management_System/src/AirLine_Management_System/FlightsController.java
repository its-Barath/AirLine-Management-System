...
public class FlightsController {
    ...
    public static void AddNewFlight(Database database, Scanner s) throws SQLException {
        ...
        flight.setID(id);
        String insert = "INSERT INTO `flights`(`id`, `airplane`, `origin`, `destination`, `departureTime`, `arrivalTime`, `isDelayed`, `bookedEconomy`, `bookedBusiness`, `stuff`, `passengers`) VALUES ('" + flight.getID() + "','" + planeID + "','" + originID + "','" + destinationID + "','" + dTime + "','" + aTime + "',0,'0','0'" + ",'<%%/>','<%%/>');";
    ...
    public static ArrayList<Flight> getAllFlights(Database database) throws SQLException {
        ...
        for(int i = 0; i < IDs.size(); ++i) {
            Flight flight = new Flight();
            flight.setID((Integer)IDs.get(i));
            int planeID = (Integer)planeIDs.get(i);
            int originID = (Integer)originIDs.get(i);
            int destID = (Integer)destIDs.get(i);
            String depTime = (String)depTimes.get(i);
            String arrTime = (String)arrTimes.get(i);
            String del = (String)dels.get(i);
            boolean delayed = Boolean.parseBoolean(del);
            flight.setBookedEconomy((Integer)bookedEconomySeats.get(i));
            flight.setBookedBusiness((Integer)bookedBusinessSeats.get(i));
            String st = (String)sts.get(i);
            String pas = (String)pass.get(i);
            Airplane plane = AirplanesController.getPlaneByID(database, planeID);
            flight.setAirplane(plane);
            ...
            for(int j = 0; j < stuffID.length; ++j) {
                totalCapacity = Integer.parseInt(stuffID[j]);
                stuff[j] = EmployeesController.getEmployeeByID(database, totalCapacity);
            }

            flight.setStuff(stuff);
            String[] passengersID = pas.split("<%%/>");
            totalCapacity = plane.getEconomyCapacity() + plane.getBusinessCapacity();
    ...
    public static void delayFlight(Database database, Scanner s) throws SQLException {
        ...
        int isDelayedValue = 1;

        String update = "UPDATE `flights` SET `isDelayed`=" + isDelayedValue + " WHERE `id` = " + id + " ;";
    ...
    public static void bookFlight(Database database, Scanner s) throws SQLException {
        ...
        Passenger[] var13;
        int var12 = (var13 = flight.getPassengers()).length;

        for(int var11 = 0; var11 < var12; ++var11) {
            Passenger p = var13[var11];
            if (p != null) {
                sb.append(p.getId()).append("<%%/>");
            }
        }

        String update = "UPDATE `flights` SET `bookedEconomy`='" + flight.getBookedEconomy() + "',`bookedBusiness`='" + flight.getBookedBusiness() + "', `passengers`='" + sb.toString() + "' WHERE `id` = " + flight.getID() + " ;";
        database.getStatement().execute(update);
    ...
    public static Flight getFlight(Database database, int id) throws SQLException {
        Flight flight = new Flight();
        String select = "SELECT `id`, `airplane`, `origin`, `destination`, `departureTime`, `arrivalTime`, `isDelayed`, `bookedEconomy`, `bookedBusiness`, `stuff`, `passengers` FROM `flights` WHERE `id` = " + id + ";";
        ResultSet rs = database.getStatement().executeQuery(select);
        rs.next();
        int ID = rs.getInt("id");
    ...
    public static void setFlightStuff(Database database, Scanner s) throws SQLException {
        ...
        Employee[] var10;
        int var9 = (var10 = flight.getStuff()).length;

        for(int var8 = 0; var8 < var9; ++var8) {
            Employee e = var10[var8];
            if (e != null) {
    ...
    public static void printFlightStuff(Database database, Scanner s) throws SQLException {
        ...
        Employee[] var7;
        int var6 = (var7 = f.getStuff()).length;

        for(int var5 = 0; var5 < var6; ++var5) {
            Employee e = var7[var5];
            if (e != null) {
    ...
    public static void printFlightPassengers(Database database, Scanner s) throws SQLException {
        ...
        Passenger[] var7;
        int var6 = (var7 = f.getPassengers()).length;

        for(int var5 = 0; var5 < var6; ++var5) {
            Passenger p = var7[var5];
            if (p != null) {
